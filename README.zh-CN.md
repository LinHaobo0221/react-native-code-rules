...
- `mobile/` 标准目录结构
- Page / UI / Hook / Data / Shared 的职责边界
- 组件、文件、Props 与样式组织规范
- Expo Router 的通用组织原则
...
使用方仓库必须明确声明 `rules_language`，并指向对应语言的 `AGENTS` 入口。不能根据设备 locale 或代码文本自动猜测语言。未声明受支持语言时回退英文。
...
每个语言目录都包含相同编号的 10 份规约文档和 1 份项目专属模板。
...
1. 当前任务中用户明确提出的需求与限制
2. 使用方仓库中更具体的 `AGENTS.md` 和 `app-specific.md`
3. 本 package 的共通规约
4. 未被上述规则覆盖时的保守工程判断
